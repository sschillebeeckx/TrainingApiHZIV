...
public class FilePersonRepository implements PersonRepository {
	...
	private String fileLoc = "/temp/javacourses/hziv/persons.csv";
		...
		try {
			...
			while ((s = br.readLine()) != null) {
				...
				if (!vals[0].equals("")) {
					...
					a.setNr(!vals[11].equals("null") ? vals[11] : null);
					a.setZipcode(!vals[12].equals("null") ? vals[12] : null);
					a.setTown(!vals[13].equals("null") ? vals[13] : null);
					a.setCountryCode(!vals[14].equals("null") ? vals[14] : null);
	...
	public void addPerson(Person p) throws PersonAlreadyExistsException {
		...
		this.readFile();
		Iterator<Person> iter = allPersons.iterator();
        try (PrintWriter pw  = new PrintWriter(new FileWriter(fileLoc, true))){
			while (iter.hasNext()) {
				Person pers = iter.next();
	...
	private StringBuilder parsePerson(Person p) {
		...
		sb.append(p.getPersonId() + ";").append(p.getFirstName() + ";").append(p.getLastName() + ";")
				...
				.append(p.getCompany().getAddress().getTown()+ ";").append(p.getCompany().getAddress().getCountryCode());
	...
	private void writePersons() {
        try ( PrintWriter pw = new PrintWriter(new FileWriter(fileLoc))){
