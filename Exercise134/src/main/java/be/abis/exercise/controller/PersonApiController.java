...
public class PersonApiController {
    ...
    @PostMapping("/login")
    public Mono<ResponseEntity<PersonDTO>> findPersonByMailAndPwd(@RequestBody Login login) throws PersonNotFoundException {
    ...
    @PutMapping("{id}")
    public Mono<PersonDTO> changePassword(@PathVariable("id") int id, @RequestBody PersonForm person,
                                          @RequestHeader("X-API-Key") String apiKey)  {
        System.out.println("changing password to newpswd= " + person.getPassword());
        return ps.changePasswordPut(id,person, person.getPassword(),apiKey);
    }
